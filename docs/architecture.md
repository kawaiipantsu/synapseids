...
| `flow` | `Key` (direction-normalized 5-tuple **scoped by observation point**: `Key.Sensor` is the sensor that saw the traffic, `""` for local capture — two sensors watching one routed conversation therefore keep two flows instead of merging into one with doubled counters, [ADR 0030](adr/0030-flow-attribution-scoped-by-observation-point.md)), `Record` (raw accumulators + derived-stat methods; `Sensor()` / `WithSensor` read and set the scope, and `WithDerivedKey` preserves it because no endpoint can say where a flow was observed), `Table` (lifecycle: open, fold, snapshot, close, evict, TIME_WAIT grace). Single-goroutine, one table, one `max_flows` cap — so eviction stays global oldest-idle across sensors. | `packet` | `features`, `inference`, `capture`, `storage`, `events`, `api` |
...
| `obs` | Observability (§24, #55, [ADR 0033](adr/0033-observability-metrics-and-structured-logging.md)). `Histogram` — a hand-rolled fixed-bucket cumulative histogram in the exact shape the Prometheus wire format needs. `Metrics` — the two latency histograms (model scoring, `features.Extract`) and the per-class verdict tally that no other `Stats()` covers; recorded by `pipeline` off the packet loop, read by `internal/api` for `GET /metrics`; nil-safe. `Writer` — the Prometheus `0.0.4` text exposition, written by hand (no client library). `SetupLogging` — builds a `log/slog` text/JSON handler at a configured level, installs it as the default, and bridges the standard `log` package so injected `log.Printf` lines land in the structured stream; returns a `*Logger` with a live `slog.LevelVar` for config hot-reload. | `schema` (+ stdlib) | `api`, `pipeline`, `cmd/synapsed` |
...
- **Wired views:** Dashboard, the full-screen Flow Log, the Flow Inspector
  ...
  **CAPTURE ▸ Sensors** (§19.15, #46 — `GET /api/v1/sensors/topology` grouped by
  ...
  location and a sensor; the daemon states per sensor whether a scope works in
  `flow_attribution`, which since #126 is `"records"` for a record-mode sensor,
  `"packets"` for a raw-mode one, and `"none"` only for a peer that reported no
  id at all — see [ADR 0026](adr/0026-traffic-matrix-and-sensor-topology.md) and
  [ADR 0030](adr/0030-flow-attribution-scoped-by-observation-point.md)), and **LIVE ▸
...
- **Detections degrades honestly on a daemon without the endpoint.**
  ...
  daemon.
- **Deliberately labelled stubs** on Hosts and Investigate for the remaining
  §19.4 gap: a *behavioural baseline* / unusual-feature callouts (#63). The API
  reports `baseline_available: false` and the SPA says so rather than inventing a
  baseline. **Per-host similarity is real** — `GET /api/v1/hosts/{ip}/similar`
  and a "Similar hosts" panel on Investigate, over a hand-crafted behavioural
  fingerprint ([ADR 0039](adr/0039-per-host-behavioural-fingerprint.md), #63); a
  learned embedding is the upgrade behind the same shape. **Anomaly scoring is real** once
...
| Location / global / experimental model roles wired end to end | `internal/nn` runs ONNX MLPs; `inference.ONNXModel` adapts them to `Classifier`; `internal/model` loads and validates the five-file bundle; `internal/registry` records it with lineage. **The anomaly role is wired** ([ADR 0037](adr/0037-anomaly-autoencoder-model.md), #47): `flow-anomaly-v1` is a second frozen family (48→48, `reconstruction-v1` output), `inference.ONNXAnomalyModel` implements the parallel `AnomalyScorer` interface, and `POST /api/v1/models/{id}/activate` is **role-aware** — it learns the role from the bundle family and swaps only within that role, so a primary classifier and an anomaly model are Active at once; `deactivate` drops that one role (`Runtime.ActivateRole` / `DeactivateRole` on top of `SetModels`/`SetAnomalyModels`). **The sequence role is wired** too ([ADR 0040](adr/0040-temporal-flow-sequence-model.md), #62): `flow-sequence-v1` is a third frozen family (a `[16, 48]` history in, `traffic-classes-v1` out) whose `ONNXSequenceModel` is a supervised peer *with memory* — its top class joins `models[]` and the disagreement set but never drives the verdict, and it activates alongside a primary + anomaly model. `internal/pipeline` keeps a bounded, lock-free per-conversation feature-vector ring. This release's model is a windowed FFN (the history flattened); a trained TCN/GRU (needs 1-D `Conv`/`GRU` in `internal/nn`) and the `synapse-trainer` objectives (`reconstruction`, `sequence`) are tracked follow-ups — the Go paths are exercised with `internal/modeltest` bundles. Location/global/experimental peers still ride `SetModels`. The offline `synapse-trainer` lives in `trainer/` ([ADR 0007](adr/0007-python-trainer-and-bundle-export.md)). | see EPIC: Phase 2 / 7 |
...
| Behavioural baselines and unusual-feature callouts on a host (§19.4-6) | Host profiles, the timeline and the **per-host anomaly aggregates** are real (`anomaly_available` flips true under a `flow-anomaly-v1` model, [ADR 0037](adr/0037-anomaly-autoencoder-model.md)); **per-host similarity** is real over a hand-crafted behavioural fingerprint (`/hosts/{ip}/similar`, [ADR 0039](adr/0039-per-host-behavioural-fingerprint.md)). Still missing: a fitted behavioural *baseline* (`baseline_available: false`, never fabricated) and a *learned* per-host embedding | #63 (learned embedding + baseline) under EPIC: Phase 7 (#7) |
