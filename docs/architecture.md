...
| `audit` | Append-only JSONL (`audit.log`, next to the bundles): one `{ts,event,actor,subject_type,subject,model_id,detail}` line per `ModelRegistered` / `ModelActivated` / `ModelDeactivated` and per `DatasetCreated` / `DatasetDerived` / `DatasetDeleted` (§21, §28.14). `Log` keeps the model shape (`subject_type:"model"`, `model_id` populated); `LogSubject` carries any subject. `actor` is `"local"` until RBAC (#58). Never blocks a request. | stdlib only | `api` |
...
| The rest of the §19 UI — Detections, Sensors, Model registry, Training, Model compare, Drift, Performance, Storage, Settings | React SPA (`web/ui/`, built into the embedded `web/dist/`) with the Dashboard, Flow Log, Flow Inspector, Replay control, Capture Sources, the Architecture builder, the Dataset **Manager** (§19.10), the Dataset **Explorer** (§19.11) and **Hosts, Investigate and Timeline** wired; every other route is a "Planned — Phase N" placeholder | see EPIC: Phase 2 / 4 / 5 / 6 / 7 (per view) |
...
| **Phase 4** — the human review loop, the training run + live training dashboard, training recipes (combining several datasets with weighting), and train/val/test planning in the UI | The **dataset manager** is done: `internal/dataset` materialises versioned, immutable, content-hashed datasets from stored classifications, `/api/v1/datasets*` exposes them, and `ML ▸ Datasets` drives it ([ADR 0015](adr/0015-versioned-datasets-on-disk.md), issue #33). The **Dataset Explorer** (§19.11) is also done — `GET /api/v1/datasets/{ref}/stats` and `ML ▸ Dataset Explorer` give feature distributions, the 48×48 correlation matrix, protocol/port splits, outliers and a stdlib-only PCA projection ([ADR 0020](adr/0020-dataset-explorer-and-in-tree-pca.md), issues #37 and #67; UMAP deferred). Splitting is reproducible but happens **in the trainer** (`synapse_trainer.dataset.Dataset.split`), not here. Because there is no review loop, every dataset's `labeling_source` is `model_prediction:<ids>` — the daemon's own predictions, never presented as ground truth. | issue #42 (review), rest of EPIC: Phase 4 |
