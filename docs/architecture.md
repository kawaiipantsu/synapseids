...
| `capture` | `Source` interface + adapters: `PCAPFile` (classic pcap plus a minimal read-only pcapng reader — SHB/IDB/EPB/SPB, Ethernet or RAW), `Replay` (paces an inner source to wall-clock × speed), `AFPacket` (Phase 3 — stdlib-only Linux `AF_PACKET` raw socket; built-in cBPF filter presets; `Stats.Drops` from `PACKET_STATISTICS`; non-Linux stub), and the subprocess sources `TcpdumpStream` / `SSHTcpdump` (Phase 3 — `tcpdump -w -` locally or over `ssh`; shared `pcapSubprocess` lifecycle, `ssh` gated on `authorized:true` per §28.18). The classic/pcapng stream decode is the shared `decodePCAPStream` engine (`pcapstream.go`), used by `PCAPFile` and both subprocess sources. `Manager` runs N sources and merges them into one stream for a single pipeline goroutine, isolating a failed source; it is itself a `Source`. `Stats` counters (incl. `Drops`). See [ADR 0010](adr/0010-live-capture-af_packet-and-the-source-manager.md), [ADR 0011](adr/0011-subprocess-capture-tcpdump-and-ssh.md). | `packet` | `flow`, `features`, `inference`, `storage`, `events`, `api`, `config` |
...
|---|---|---|
| Live capture — PCAP-over-IP; runtime add/remove of sources; a subprocess supervisor / auto-restart; SSH dial-latency and tcpdump kernel-drop parsing; live-capture flow-table stats on `/api/v1/status` | **Local NIC, local tcpdump-stream and SSH remote-tcpdump capture are wired** (`capture.AFPacket`, `capture.TcpdumpStream`, `capture.SSHTcpdump` + `capture.Manager`; `synapsed --capture <iface>` / `capture.sources[]` with `kind` `nic` / `tcpdump` / `ssh`; `GET /api/v1/captures`; [ADR 0010](adr/0010-live-capture-af_packet-and-the-source-manager.md), [ADR 0011](adr/0011-subprocess-capture-tcpdump-and-ssh.md)). `nic` filters are built-in cBPF presets (`ip`, `ip6`, `ip-any`, `not-arp`) only; `tcpdump` / `ssh` take a raw tcpdump filter expression. A `kind:"ssh"` source requires `"authorized": true` (§28.18). No auto-restart on subprocess exit. `capture.PCAPFile` + `capture.Replay` still handle classic pcap and minimal pcapng. | see EPIC: Phase 3 (#28–#30 done; #31–#32 open) |
