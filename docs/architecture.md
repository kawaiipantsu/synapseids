...
| The rest of the §19 UI — Detections, Sensors, Model registry, Model compare, Drift, Performance, Storage, Settings | React SPA (`web/ui/`, built into the embedded `web/dist/`) with the Dashboard, Flow Log, Flow Inspector, Replay control, Capture Sources, the Architecture builder, the Dataset **Manager** (§19.10), the Dataset **Explorer** (§19.11), the **Training dashboard** (§19.8, polls `/api/v1/training/{id}`), and **Hosts, Investigate and Timeline** wired — with **downloadable investigation reports** (HTML/JSON) from Investigate and each Hosts row (#66); every other route is a "Planned — Phase N" placeholder | see EPIC: Phase 2 / 4 / 5 / 6 / 7 (per view) |
...
| YAML config, retention sweeper, detection API group | JSON config; rings are size-bounded, not time-bounded. The host/investigation API group now exists (`/api/v1/hosts*`, `/api/v1/timeline`, and `/api/v1/reports/*` — **downloadable investigation reports are done**, issue #66 / [ADR 0023](adr/0023-downloadable-investigation-reports.md)) but its aggregates are process-lifetime and volatile, like `storage.Mem`; a report says so in its own coverage block rather than pretending otherwise. Report *history*, scheduled export and e-mail delivery are not built | see EPIC: Phase 2 ([ADR 0002](adr/0002-flow-features-v1-frozen-and-json-config.md)), Phase 5 |
