...
| The rest of the §19 UI — Investigate, Hosts, Detections, Sensors, Model registry, Training, Datasets, Model compare, Drift, Performance, Storage, Settings | React SPA (`web/ui/`, built into the embedded `web/dist/`) with the Dashboard, Flow Log, Flow Inspector, Replay control, Capture Sources and the Architecture builder wired; every other route is a "Planned — Phase N" placeholder | see EPIC: Phase 2 / 4 / 5 / 6 / 7 (per view) |
