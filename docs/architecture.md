...
- **Wired views:** Dashboard, the full-screen Flow Log, the Flow Inspector
  ...
  `schema.Architecture` export). Every other §19 route is a "Planned — Phase N"
  placeholder.
...
| The rest of the §19 UI — Investigate, Hosts, Detections, Sensors, Model registry, Training, Dataset **Explorer**, Model compare, Drift, Performance, Storage, Settings | React SPA (`web/ui/`, built into the embedded `web/dist/`) with the Dashboard, Flow Log, Flow Inspector, Replay control, Capture Sources, the Architecture builder and the Dataset **Manager** (§19.10) wired; every other route is a "Planned — Phase N" placeholder | see EPIC: Phase 2 / 4 / 5 / 6 / 7 (per view) |
...
| Anomaly model, model compare, drift | — | see EPIC: Phase 5, Phase 7 |
