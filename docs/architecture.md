...
- **Detections degrades honestly.** `GET /api/v1/detections` is landing
  separately (#117); until it does, `client.getDetections` maps a 404 to
  `state: 'unavailable'` and both the route and the Dashboard card say "not
  available in this build", once, with no retry loop, no spinner and no console
  error. The same code lights up unchanged when the endpoint appears; it is
  verified against `web/ui/test/fixtures/detections.json`, a byte-for-byte
  instance of the agreed contract.
...
| Detections — the `/api/v1/detections` resource, an alert store, anything publishing `AlertCreated` | The **SPA half is done** (#118): `LIVE ▸ Detections` and the Dashboard's *Recent detections* card are built against the fixed response contract and unit-tested against `web/ui/test/fixtures/detections.json`. Nothing server-side emits `AlertCreated` yet, so on this build the endpoint 404s and both surfaces render "not available in this build" — a state, not an error, and not a spinner. They light up with no further SPA change once the endpoint lands | #117 |
