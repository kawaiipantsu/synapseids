...
- **Wired views:** Dashboard, the full-screen Flow Log, the Flow Inspector
  ...
  **CAPTURE ▸ Sensors** (§19.15, #46 — `GET /api/v1/sensors/topology` grouped by
  ...
  [ADR 0026](adr/0026-traffic-matrix-and-sensor-topology.md)). Every other §19
  route is a "Planned — Phase N" placeholder.
- **The detections backend now exists** (#117): `GET /api/v1/detections[/{id}]`,
  `alerts.*` on `/api/v1/status`, and a real `AlertCreated` on the live channel
  ([ADR 0027](adr/0027-detection-dedup-and-derived-severity.md)). The SPA view is
...
| The rest of the §19 UI — Detections, Model compare, Drift, Performance, Storage, Settings | React SPA (`web/ui/`, built into the embedded `web/dist/`) with the Dashboard, Flow Log, Flow Inspector, Replay control, Capture Sources, the Architecture builder, the Model **registry** (§19.12), the Dataset **Manager** (§19.10), the Dataset **Explorer** (§19.11), the **Training dashboard** (§19.8, polls `/api/v1/training/{id}`), **Hosts, Investigate, Timeline and Review**, and now the **traffic Matrix** (§19.4-5, #68) and **Sensors topology** (§19.15, #46) wired; every other route is a "Planned — Phase N" placeholder | see EPIC: Phase 2 / 7 (per view) |
| ~~Detections — an `/api/v1/detections` resource, an alert store, anything publishing `AlertCreated`~~ | **Backend done (#117, [ADR 0027](adr/0027-detection-dedup-and-derived-severity.md)).** `internal/alert` is an alert `Policy` (per-class confidence thresholds + `alert_on_disagreement`) plus a bounded, **deduplicating** detection store keyed on `(src_ip, dst_ip, class)` in a window anchored at the first occurrence — a 1000-port sweep is one detection with `count: 1000` and **one** `AlertCreated`, not a thousand of each (§22). `GET /api/v1/detections` filters on `class` / `severity` / `min_confidence` / `since` / `limit` and reports `total` / `returned` / `evicted`; `GET /api/v1/detections/{id}` is the single object. `severity` is *derived* from the class (`normal` never alerts; `suspicious`→`low`, `scan`→`medium`, `brute_force`/`web_attack`→`high`, `dos_ddos`/`botnet_c2`→`critical`) and covering the frozen class list is a startup gate, not a silent `""`. Counters on `/api/v1/status` under `alerts`. **Not done:** notification delivery (email/webhook/syslog), anomaly-driven detections (#47), per-detection acknowledge/close state, and persistence — detections live in a 1000-entry ring and do not survive a restart. **The feed's quality is the heuristic's:** on a real nmap+brute-force capture the classifier misses the recon entirely (#90), so there is no `scan` detection at all; the thresholds were not tuned to hide it. | #117 **closed**; notification delivery and #47 remain |
