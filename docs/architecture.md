...
| `audit` | Append-only JSONL (`audit.log`, next to the bundles): one `{ts,event,actor,subject_type,subject,model_id,detail}` line per `ModelRegistered` / `ModelActivated` / `ModelDeactivated`, per `DatasetCreated` / `DatasetDerived` / `DatasetDeleted`, per `TrainingStarted` / `TrainingCompleted` / `TrainingFailed`, and one `ReviewUpdated` line per human label change (§21's "human label changes", §16, §28.14-15). `Log` keeps the model shape (`subject_type:"model"`, `model_id` populated); `LogSubject` carries any subject (`model` / `dataset` / `training` / `review`). `actor` is `"local"` until RBAC (#58). Never blocks a request. | stdlib only | `api`, `training`, `review` |
