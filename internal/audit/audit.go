// Package audit writes an append-only record of model-lifecycle actions
// (PROJECT.md §21, §28.14): who registered, activated or deactivated which model
// and when. It is deliberately tiny — one JSON object per line, appended to a
// single file — so it adds no dependency and never meaningfully blocks the API
...
type Record struct {
	...
	ModelID string `json:"model_id"` // the model the action concerns
...
// Log appends one record with the current UTC timestamp and mirrors it to the
// structured log. A write error is logged, never returned: an audit-log failure
...
func (l *Logger) Log(event, actor, modelID, detail string) {
	...
	if err != nil { // cannot happen for these fields, but never panic
		l.logf("audit: marshal %s for %q: %v", event, modelID, err)
		return
	}
	...
	l.logf("audit: %s actor=%s model=%s %s", event, actor, modelID, detail)
}
